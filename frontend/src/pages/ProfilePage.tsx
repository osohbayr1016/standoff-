...
export default function ProfilePage({
  ...
  const [showAlliesHistory, setShowAlliesHistory] = useState(false);
  const [alliesMatches, setAlliesMatches] = useState<MatchHistoryItem[]>([]);
  const [loadingAlliesHistory, setLoadingAlliesHistory] = useState(false);
  ...
  const fetchAlliesMatches = async () => {
    if (!profileId) return;
    setLoadingAlliesHistory(true);
    try {
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL || "http://localhost:8787"}/api/profile/${profileId}/matches?type=allies`
      );
      if (res.ok) {
        const data = await res.json();
        setAlliesMatches(data.matches || []);
      }
    } catch (err) {
      console.error("Failed to fetch allies matches:", err);
    } finally {
      setLoadingAlliesHistory(false);
    }
  };

  useEffect(() => {
    if (showAlliesHistory && profileId) {
      fetchAlliesMatches();
    }
  }, [showAlliesHistory, profileId]);

  const handleLeaveClan = async () => {
    if (!user || !profile?.clan) return;
    if (!confirm('Are you sure you want to leave the clan?')) return;
    setLeavingClan(true);
    try {
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL || "http://localhost:8787"}/api/clans/members/kick`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      if (res.ok) {
        setSuccessMsg('Left clan successfully');
        // Refresh profile
        fetchProfile(profileId!);
      } else {
        const err = await res.json();
        setError(err.error || 'Failed to leave clan');
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#0E0F12] text-white font-sans selection:bg-[#ff5500] selection:text-white pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-4 md:py-8 grid grid-cols-1 lg:grid-cols-4 gap-6 md:gap-8">
        ...
        <div className="lg:col-span-3 space-y-6 md:space-y-8">
          ...
          <div className="flex items-center border-b border-white/10 mb-4 md:mb-6 overflow-x-auto">
            ...
            <button className="px-4 md:px-6 py-3 md:py-4 text-[#555] font-bold uppercase tracking-wider hover:text-white transition-colors text-xs md:text-base whitespace-nowrap" onClick={() => setShowAlliesHistory(true)}>
              Allies Matches
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {/* Allies Stats Cards */}
            <Card
              className="bg-[#121418] border-none rounded-none border-l-2 border-orange-500 p-4 md:p-6 shadow-sm"
            >
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h3 className="text-[#9ca3af] font-bold uppercase tracking-widest text-xs mb-1">Allies ELO</h3>
                  <p className="text-3xl font-black text-white italic">{profile?.allies_elo || 1000}</p>
                </div>
                <div className="h-10 w-10 text-orange-500">
                  <Swords className="h-full w-full opacity-20" />
                </div>
              </div>
              <p className="text-[10px] text-[#555] mt-2 font-bold uppercase">2v2 Competitive Rating</p>
            </Card>
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h3 className="text-[#9ca3af] font-bold uppercase tracking-widest text-xs mb-1">Allies Matches</h3>
                  <p className="text-3xl font-black text-white italic">{(profile?.allies_wins || 0) + (profile?.allies_losses || 0)}</p>
                </div>
                <div className="h-10 w-10 text-orange-500">
                  <Trophy className="h-full w-full opacity-20" />
                </div>
              ...
              <div className="flex gap-2 text-sm font-bold">
                <span className="text-green-500">{profile?.allies_wins || 0} Wins</span>
                <span className="text-white/20">|</span>
                <span className="text-red-500">{profile?.allies_losses || 0} Losses</span>
              </div>
            </Card>
          </div>
        ...
        {/* RIGHT Column: Clan & Sidebar */}
        <div className="hidden lg:block space-y-6">
          {/* Clan Card */}
          {profile?.clan ? (
            <Card className="bg-[#121418] border border-white/5 overflow-hidden">
              <div className="p-4 border-b border-white/5">
                <h3 className="font-bold uppercase tracking-widest text-xs text-[#9ca3af]">Clan</h3>
              </div>
              <div className="p-4 flex items-center gap-4">
                {profile.clan.logo_url ? (
                  <img
                    src={profile.clan.logo_url}
                    alt={profile.clan.name}
                    className="w-14 h-14 rounded-lg object-cover border border-white/10"
                  />
                ) : (
                  <div className="w-14 h-14 rounded-lg bg-gradient-to-br from-orange-500/30 to-red-500/30 flex items-center justify-center text-xl font-black text-white/50">
                    {profile.clan.tag}
                  </div>
                )}
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-black text-white text-lg truncate">{profile.clan.name}</span>
                    <span className="text-xs bg-white/10 px-1.5 py-0.5 rounded font-mono text-white/60">[{profile.clan.tag}]</span>
                  </div>
                  <div className="flex items-center gap-2 text-xs text-white/50 mt-1">
                    <span className="capitalize">{profile.clan.member_role || 'Member'}</span>
                    <span>•</span>
                    <span>{profile.clan.member_count || 1} members</span>
                  </div>
                </div>
              </div>
              {profile.clan.description && (
                <div className="px-4 pb-4">
                  <p className="text-xs text-white/40 line-clamp-2">{profile.clan.description}</p>
                </div>
              )}
              {/* Leave Clan Button - Only for own profile and not leader */}
              {isOwnProfile && profile.clan.member_role !== 'leader' && (
                <div className="px-4 pb-4">
                  <Button
            ...
            <Card className="bg-[#121418] border border-white/5 p-6 text-center">
              <div className="text-white/30 font-bold uppercase text-xs tracking-widest mb-2">No Clan</div>
              <p className="text-xs text-white/20">This player is not in a clan</p>
            </Card>
          )}
        </div>
      ...
      {/* Allies Matches Dialog */}
      <Dialog open={showAlliesHistory} onOpenChange={setShowAlliesHistory}>
        <DialogContent className="bg-[#1c1e22] border-[#ff5500]/20 text-white max-w-3xl max-h-[75vh] flex flex-col">
          <DialogHeader className="p-6 pb-2 border-b border-white/5">
            <DialogTitle className="flex items-center gap-3 text-xl font-black uppercase tracking-wider">
              <Users className="h-5 w-5 text-green-500" />
              Allies Match History
            </DialogTitle>
            <DialogDescription>
              2v2 Allies matches. ELO changes apply to your Allies rank.
            </DialogDescription>
          </DialogHeader>

          <div className="flex-1 overflow-y-auto p-6 pt-0">
            {loadingAlliesHistory ? (
              <div className="flex items-center justify-center py-12">
              ...
              <Table>
                <TableHeader className="bg-[#121418] sticky top-0 z-10">
                  <TableRow className="border-white/5 hover:bg-transparent">
                    ...
                    <TableHead className="text-center text-xs font-bold uppercase tracking-wider text-muted-foreground bg-[#121418]">Result</TableHead>
                    <TableHead className="text-center text-xs font-bold uppercase tracking-wider text-muted-foreground bg-[#121418]">Score</TableHead>
                    <TableHead className="text-right text-xs font-bold uppercase tracking-wider text-muted-foreground bg-[#121418]">ELO</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {alliesMatches && alliesMatches.length > 0 ? (
                    alliesMatches.map((match) => {
                      const isWinner = match.winner_team === match.player_team;
                      ...
                      return (
                        <TableRow key={match.match_id} className="border-white/5 hover:bg-white/5 transition-colors">
                          ...
                          <TableCell className="text-right">
                            <span className={`font-mono font-bold text-sm ${eloChange >= 0 ? 'text-green-500' : 'text-red-500'}`}>
          ...
          <DialogFooter className="p-6 pt-2 border-t border-white/5">
            <Button onClick={() => setShowAlliesHistory(false)} variant="ghost">Close</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      {user && (
        <BannerEditModal
          ...
          onSave={(newUrl: string, newMode?: 'discord' | 'custom') => {
