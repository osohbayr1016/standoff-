...
const InviteFriendModal = lazy(() => import('../features/lobby/InviteFriendModal'));
const VoiceChat = lazy(() => import('../components/VoiceChat'));
import LobbyWorker from '../workers/lobby.worker?worker';
import { DraftPhase } from '../features/lobby/DraftPhase';
import { CompetitiveLobbyWaiting } from '../features/lobby/CompetitiveLobbyWaiting';

import type { Match, MatchPlayer } from '../types/match';
import { LobbyActionButtons } from '../features/lobby/LobbyActionButtons';
import { TeamColumn } from '../features/lobby/TeamColumn';
...
const LobbyDetailPage: React.FC<LobbyDetailPageProps> = ({ matchId, user, backendUrl, onBack, previousProfileUserId, onNavigateToProfile }) => {
    ...
    }, [user, backendUrl, matchId, fetchMatchDetails]);
    ...
    }, [user, backendUrl, matchId, onBack]);
    ...
    }, [user, match, matchId, backendUrl, fetchMatchDetails]);
    ...
    }, [user, match, matchId, backendUrl, onBack]);
    ...
    return (
        <div className="space-y-6 container mx-auto max-w-7xl animate-fade-in pb-12">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-9 gap-6 items-start lg:max-w-7xl lg:mx-auto">
                    ...
                    <div className="lg:col-span-4">
                        <TeamColumn
                            ...
                            emptySlots={(match.match_type === 'allies' ? 2 : 5) - alphaPlayers.length}
                            isAllies={match.match_type === 'allies'}
                    ...
                    <div className="lg:col-span-4">
                        <TeamColumn
                            ...
                            currentUserId={user?.id}
                            onKick={handleKickPlayer}
                            emptySlots={(match.match_type === 'allies' ? 2 : 5) - bravoPlayers.length}
                            isAllies={match.match_type === 'allies'}
                            onNavigateToProfile={onNavigateToProfile}
                        />
                    </div>
            ...
            <div className="mt-6 -mx-4 md:-mx-8 lg:col-span-9 lg:mx-0">
                        ...
                        {match.status === 'in_progress' && (isStaff || isHost || isCaptain) && (
                            <div className="w-full flex flex-col md:flex-row items-center gap-4">
                                <div className="flex-1 w-full p-4 bg-muted/30 rounded-lg border border-border/50">
                                    {match.match_type === 'league' || match.match_type === 'competitive' || match.match_type === 'clan_war' || match.match_type === 'allies' ? (
                                        <>
                ...
                {/* Lobby Chat */}
                <div className="mt-6">
                    <Chat lobbyId={matchId} variant="inline" title="Lobby Chat" />
                </div>
